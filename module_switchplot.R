...
# Server module
switchPlotServer <- function(id, switch_data, search_term) { # without alpha and IF arguments
  moduleServer(
    id = id,
    module = function(input, output, session) {
      ...
      for(cancerName in c("Prostate","Glioblastoma","Melanoma","Lung")){
        ...
        if (nrow(checkTable) > 0){
          ...
          printText <- paste0("No significant switch isoform for ",search_term," in ",cancerName,".")
